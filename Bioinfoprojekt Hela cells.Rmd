...
rnase_matrix=cbind(MS_Table[, seq(2,150,2)])
...
sum1 <- rowSums(norm.ctrl1)
scaled1 <- sum1 / 100
scaledctrl1 <- norm.ctrl1[1,] / scaled1


for (i in 1:nrow(norm.ctrl1)){
  scaled1 <- sum(norm.ctrl1[i, ]) / 100
  norm.ctrl1[i, ] = norm.ctrl1[i,] / scaled1
}
# normalization over the sum of 100% 
for (i in 1:nrow(norm.ctrl1)) 
  xx[i, ] <- sum(norm.ctrl1[i, ])/((sum(norm.ctrl1[i, ])/ 100))
          return(xx)

#scale <- function(x) {
  sum(norm.ctrl1[1, ])/((sum(norm.ctrl1[1, ])/ 100))
}
scale(norm.ctrl1)
# doppelte klammern bei liste 
#r studio clear environment
#nur auf eine spalte probieren das scaling
vw <- sapply(xx, scale)
View(vw)
print(barplot(vw))
...
```{r}
# remove all-zero coloumns
```
>>>>>>> Stashed changes

```{r}
#Rnase/control *100 / sum for scaling scaling
```
