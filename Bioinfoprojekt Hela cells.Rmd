...
>>>>>>> 3d6e61819e74ff2a4c993adc64d3c2b2a8aab65e
>>>>>>> Stashed changes
ctrl12=c()
ctrl13=c()
ctrl23=c()
...
```{r}
#findpeaks(datanormctrl1,ignore_threshold = 0, span = 3, strict = TRUE, na.rm = FALSE)
#pracma package?
...
ctrl_norm_mean$maxima <- apply(datanormctrl1, 1, function(x) {
															   list <- find_peaks(x)
															   list <- list[x[list] > 2] 
															   list <- unlist(list)
															   list
...
#Unser Code
find_peaks <- function(x, m=2) { #in this case m was set to m=2 to start off, but will be adjusted accordingly after first results
...
#If one of the vectors of either Control or RNase has a row with multiple values, it should compare the amount of the proteins in said positions and pick the position with the highest protein amount

if(x[,length(x) > 1]) return(max(x[i]))

View(find_sig_peaks(datanormctrl1, threshold = 3))
View(find_sig_peaks(datanormrnase1, threshold = 3))

datanormctrl1["AT2A3_HUMAN",]
datanormrnase1["AT2A3_HUMAN",]
