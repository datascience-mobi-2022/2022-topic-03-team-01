...
#b) k-means 
clus5.RDP=c() #creating vector containing all RDPs identified by k-means in cluster 5
for (i in 1:nrow(mtrx.kmeans)){
if(clus5$cluster[i]==5){
  clus5.RDP=append(clus5.RDP, rnames[i]) #length =551
}}

#find similar proteins between cluster5 and RDP_Table. Those are true positives.
true.pos.clus5=c()
true.pos.pos.clus5=c()
false.pos.clus5=c()
for (i in 1:length(clus5.RDP)){
  for (j in 1:nrow(RDP_Table)){
    if(clus5.RDP[i]==RDP_Table[j,1]){
      true.pos.clus5=append(true.pos.clus5,clus5.RDP[i]) #length=424 --> = true positives
      true.pos.pos.clus5=append(true.pos.pos.clus5, i) #position of true.pos in clus5.RDP
    }}}

false.pos.clus5=clus5.RDP[-true.pos.pos.clus5] #length =127 --> = false positives

nonRDP.clus5= rnames[-clus5.RDP.indices] #length = 4105

false.neg.clus5.indices=c()
true.RDP=RDP_Table[,1]
for (i in 1:length(nonRDP.clus5)){
  false.neg.clus5.indices = append(false.neg.clus5.indices, c(grep(nonRDP.clus5[i],true.RDP, value=FALSE)))
} # 2634 false negatives 

true.neg.clus5.indices=length(nonRDP.clus5)-length(false.neg.clus5.indices) # 1471 true negatives

false.pos.rate.clus5=length(false.pos.clus5)/(true.neg.clus5.indices+length(false.pos.clus5)) # fals pos rate=7,95%
false.neg.rate.clus5=length(false.neg.clus5.indices)/(length(true.pos.clus5)+length(false.neg.clus5.indices))# false neg rate = 85,2% 
