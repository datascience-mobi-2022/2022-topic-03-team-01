...
#Split dataset into 2 datasets. One dataset consists only of Control group and the other of RNase.
#nrow(ctrl.matrix)=4765
ctrl.matrix= cbind(MS_Table[, seq(1,150,2)]) #A Matrix with all Control values from MS_Table is assigned
rnase.matrix=cbind(MS_Table[, seq(2,150,2)]) #same thing, but with all RNase values

#Removing all zero rows
crep1=cbind(ctrl.matrix[,seq(1,75,3)])  #A Matrix with all control values from all 25 fractions from rep1 is assigned
crep2=cbind(ctrl.matrix[,seq(2,75,3)])  #Same thing, but Control - Rep2 and eventually for Rep3 and same thing with RNase
...
#allzero contains 33 proteins that are deleted from the original reps
rmv.rows.crep1 = which(apply(crep1,1,function(x){sum(x)})==0) #"which" will indicate the rows, on which the function will be applied that checks, which row has a total sum equal to "0" and thus equals TRUE
...
allzero=c() #new empty vector called allzero, to which the row positions given in rmv.rows will be added to
...
ctrl.matrix=ctrl.matrix[-allzero,]  #then we remove said rows/proteins by deleting "allzero" from ctrl. and rnase.matrix
rnase.matrix=rnase.matrix[-allzero,]

#########################################################################################################################

#Normalization
ctrl.frc1=cbind(ctrl.matrix[,1:3])  #we create variables for each fraction on the three reps, both for control and RNase
...
mean.ctrl.frc1=apply(ctrl.frc1,2,function(x){mean(x)})  #we calculate the mean of the three reps on the same fraction, by creating an apply function that iterates over all proteins
...
norm.mean = function(x) {
  diff.rep1.rep2=abs(x[1]-x[2]) #we assign arithmetic subtraction of one rep from the other for all possible combinations
...
norm.fact.ctrl.frc1=norm.mean(mean.ctrl.frc1)/mean.ctrl.frc1  #norm.mean function applied to the first fraction and divided by the mean of the same fraction
...
#normed values 
norm.ctrl.frc1=data.frame(mapply('*', ctrl.frc1, norm.fact.ctrl.frc1, SIMPLIFY=FALSE))  #values are normed by multiplying the first fraction to the normalization factor of said fraction. Function iterates over all proteins
...
for (j in 1:nrow(norm.ctrl1)) #for j in 1 to the last row/protein
  for (i in 2:24) #for i in 2 to the 24th fraction 
   norm.ctrl1[j,i]=mean(c(norm.ctrl1[j,i-1],norm.ctrl1[j,i], norm.ctrl1[j,i+1]))  #this function puts new mean values in place of the old ones by calculating the mean of the to be determined value of a fraction of a protein and it's left and right neighbor
...
datanormctrl1 <- as.data.frame(norm.ctrl1)  #turn matrix into dataframe by using "as.data.frame"

for (i in 1:nrow(datanormctrl1)) {  #for i from the first protein to the last of the dataframe
  x <- sum(datanormctrl1[i, ]) / 100  #define variable x as the sum of all fractions of a proteins and divide them by 100
  datanormctrl1[i, ] = datanormctrl1[i, ] / x  #the very same rows of proteins then get divided by x and placed in the dataframe
}

rownames(datanormctrl1)=rownames(ctrl.matrix) #to make sure the dataframe has the same protein names in their place as the original matrix
...
#Reproducibility 
#amount of proteins before reproducubulity = 4732
datac1=t(datanormctrl1) #we transpose the dataframes to make them more accesible for the calculation of the correlation
...
r=0.7  #we set 0.7 or 70% as our threshold for correlating proteins

ctrl12=c()  #we create empty vectors for each of the possible correlation combinations
ctrl13=c()
ctrl23=c()
rnase12=c()
rnase13=c()
...
low.cor=function(x,a,b){  #we create a function with elements "x", "a" and "b"
  for (i in 1:ncol(datac1)){  #for i in 1 to the last column (transposed) of datac1
    x=append(x, (cor(a[,i], b[,i])))  #we append to the variable x, a variable x with the values of the correlations between a and b
  }
   r=0.7
   which(x<=r)  #then all positions where x is smaller or equal to r=0.7 shall be marked
}

low.cor(ctrl12,datac1, datac2)  #"low.cor" function is applied to all possible combinations
...
ctrl.low.cor=c()  #empty vector created, where all low correlating proteins will be moved to
...
rmv.low.cor=unique(c(ctrl.low.cor,rnase.low.cor)) #we make sure we remove rows only once, by using "unique" on ctrl. and rnase.low.cor

rmv.low.cor.ctrl=c()  #empty vector for the low correlating values of control
...
#76 proteins deleted --> 4656 proteins left

#we remove the low correlating proteins from the dataframes
...
for (j in 1:nrow(datanormctrl1)){
  for (i in 1:25){
  mean.rep.ctrl[j,i]=mean(c(datanormctrl1[j,i],datanormctrl2[j,i], datanormctrl3[j,i])) #mean of every fraction of every rep is calculated and placed in the new matrix mean.rep.ctrl
}}
...
find_peaks <- function(x, m=50) { #in this case m was set to m=2 to start off, but will be adjusted accordingly after first results
  peaks_pos <- sapply(1:length(x), FUN = function(i) {  #in peaks_pos values are going to be placed of the positions of the peaks
    if (x[i] == 0) return (numeric(0))  #if the to be determined value is equal to 0 it will remain a 0
    lborder <- max(c(1, i-m)) #left border makes sure we stay inside our fractions borders by always choosing the bigger out of 1 and i-m
    rborder <- min(c(i+m, length(x))) #right border does the same on the right side by choosing the smaller one between i+m and length(x)=25
    if(all(x[lborder:rborder] <= x[i])) return(i) else return(numeric(0)) #if all of the neighboring values are smaller/equal to the to be determined value then return the value, if not return 0
                      }
               )
...
find_sig_peaks <- function(x, threshold = 3) {  #we create a function with the elements x and threshold
        apply(x, 1, function(x) {
          peaks <- find_peaks(x)  #to the variable peaks, all values after using the find_peaks function
          peaks <- peaks[x[peaks] > threshold]  #if the values in peaks are bigger than our threshold then keep them in peaks
          unlist(peaks)
          peaks
             }
        )
...
#find_sig_peaks used on mean.rep.ctrl and rnase
peaks.ctrl = find_sig_peaks(mean.rep.ctrl, threshold = 3)
peaks.rnase = find_sig_peaks(mean.rep.rnase, threshold = 3)

peaks.ctrl.mtrx <- as.matrix(peaks.ctrl)  #turn the peak values calculated into a matrix
...
shift.mean=peaks.ctrl.mtrx-peaks.rnase.mtrx #shift is calculated by subtracting the rnase peak values from the control peak values
...
RDP.matrix=cbind(RDP.name, RDP.pos) #matrix is created with the RDP names and the positions
...
mean.sil2.width=mean(sil2.data$sil_width) #0.545
...
mean.sil3.width=mean(sil3.data$sil_width) #0.487
...
mean.sil4.width=mean(sil4.data$sil_width) #0.511

#clustering with 5 clusters 
clus5=kmeans(mtrx.kmeans, 5)
fviz_cluster(clus5,mtrx.kmeans,xlab="Max Fraction Control",ylab="Max Fraction RNase", geom="point")
...
mean.sil5.width=mean(sil5.data$sil_width) #0.528
...
mean.sil6.width=mean(sil6.data$sil_width) #0.548
...
dgfffb <- for (i in 1:nrow(shift)) {
            if (shift[i] > 0) {
                shift.type <- c(shift.type,"left shift")
             } else if (shift[i] < 0) {
                shift.type <- c(shift.type,"right shift")
            } else {
                shift.type <- c(shift.type,"no shift")
            }
}

shift.kmeans = mtrx.kmeans
kmshift=as.data.frame(shift.kmeans)
kmshift$shift.type = factor(shift.type)
clusneu =kmeans(mtrx.kmeans,5)

clusneu1 = clusneu$cluster
kmshift$cluster = factor(clusneu1)
ggscatter(
  kmshift, x= "V1", y="V2",
          color = "cluster", palette = c("red", "blue", "green", "orange", "purple", "pink"), ellipse = TRUE, ellipse.type = "convex",
          shape = "shift.type", size = 2, legend = "right", ggtheme = theme_grey(), xlab = "MaxControl", ylab = "MaxRNase" )+
...
#Linear Regression 
#model1 with proteins 1-4190 (90%): predicts amount of fractions for a shift by correlation 
corlm=c()
...
#try how the model1 works for the last 10% 
...
#Linear Regression 
#model2 with proteins 1-4190 (90%): predicts whether there is a shift or not by correlation 
corlm=c()
  for (i in 1:(0.9*ncol(t.mean.rep.ctrl))){
    corlm=append(corlm, (cor(t.mean.rep.ctrl[,i], t.mean.rep.rnase[,i])))
  }
shift.mean01=c()
for (i in 1:length(shift.mean)){
  if (shift.mean[i] != 0){
    shift.mean01=append(shift.mean01,1)
  }else{
    shift.mean01=append(shift.mean01, shift.mean[i])
  }
}

lm.data2=data.frame(shift.mean01[1:(0.9*ncol(t.mean.rep.ctrl))],corlm) 
colnames(lm.data2)=c("Shift", "Corr")
model2=lm(Shift ~ Corr, data=lm.data2)

#try how the model2 works for the last 10% percent 
cortry2=c()
  for (i in (0.9*ncol(t.mean.rep.ctrl)):ncol(t.mean.rep.ctrl)){
    cortry2=append(cortry2, (cor(t.mean.rep.ctrl[,i], t.mean.rep.rnase[,i])))
  }
shift.model2=(cortry2-model2$coefficients[1])/model2$coefficients[2]

real.shift2=c()
for (i in (0.9*ncol(t.mean.rep.ctrl)):ncol(t.mean.rep.ctrl)){
  if (shift.mean[i] != 0){
    real.shift2=append(real.shift2,1)
  }else{
    real.shift2=append(real.shift2, shift.mean[i])
  }
}
#estim.shift is a dataframe containing 0 or 1 for no shift or shift and estim. shift or not from the model2
estim.shift2=data.frame(real.shift2,shift.model2)

#expand estim.shift with column that shows shift or not. Threshold is set to 0.2, meaning there is a shift
t=0.2
est.shift2=c()
for (i in 1:length(shift.model2)){
  if (shift.model2[i] >t){
    est.shift2=append(est.shift2,1)
  }else{
    est.shift2=append(est.shift2, 0)
  }
}
estim.shift2=data.frame(estim.shift2, est.shift2)

#calculating false positive, false negative, true positiv and true negative for model2
false.pos2=0 #40
for (i in 1:length(shift.model2)){
  if (estim.shift2[i,1]<estim.shift2[i,3]){
    false.pos2=false.pos2+1
  }}
  
false.neg2=0 #39
for (i in 1:length(shift.model2)){
  if (estim.shift2[i,1]>estim.shift2[i,3]){
    false.neg2=false.neg2+1
  }}

true.pos2=0 #55
for (i in 1:length(shift.model2)){
  if (sum(estim.shift2[i,1],estim.shift2[i,3])==2){
    true.pos2=true.pos2+1
  }}

true.neg2=0 #332
for (i in 1:length(shift.model2)){
  if (sum(estim.shift2[i,1],estim.shift2[i,3])==0){
    true.neg2=true.neg2+1
  }}

false.pos.rate= false.pos2/(false.pos2+true.neg2)
false.neg.rate= false.neg2/(false.neg2+true.pos2)

########################################################################################################################

#Comparison of different methods to identify RDPs
#a) shift information and t-test
RDP_Table <- read.table("/Users/gretapick/Documents/Studium/4. Semester/Bioinformatik/RBP_Table.csv", header=TRUE, row.names=NULL, sep=";")
RDP_Table=RBP_Table[-which(RBP_Table[,2]==""),]

#find similar proteins between sigproteins and RDP.matrix. sigproteins contain also right-shifted proteins that are not RDPs, those will be eliminated. Also only RDPs with a significant shift will be chosen.
sig.RDP=c()
for (i in 1:length(sigproteins)){
  for (j in 1:nrow(RDP.matrix)){
    if(sigproteins[i]==RDP.matrix[j,1]){
      sig.RDP=append(sig.RDP,RDP.matrix[j])   #length=401
    }}}

#find similar between sig.RDPs and RDP_Table. Those are true positives.
true.pos.RDP=c()
true.pos.pos.RDP=c()
false.pos.RDP=c()
for (i in 1:length(sig.RDP)){
  for (j in 1:nrow(RDP_Table)){
    if(sig.RDP[i]==RDP_Table[j,1]){
      true.pos.RDP=append(true.pos.RDP,sig.RDP[i])#length=304 --> = true.pos
      true.pos.pos.RDP=append(true.pos.pos.RDP, i)#position of true.pos in sig.RDP
    }}}

false.pos.RDP=sig.RDP[-true.pos.pos.RDP] #length =97 --> = false positives

true.neg.RDP=c()
RDPindices=c()
for (i in 1:length(sig.RDP)){
  RDPindices=append(RDPindices,grep(sig.RDP[i],rnames)) #length=410?????
}
nonRDP=rnames[-RDPindices] #length=4246

false.neg.RDP.indices=c()
true.RDP=RDP_Table[,1]
for (i in 1:length(nonRDP)){
  false.neg.RDPindices = append(false.neg.RDP.indices, grep(nonRDP[i],true.RDP))
}

RDPindices=c() #creating vector with row number of RDPs to delete them afterwards and have only nonRDPs left
for (i in 1:length(sig.RDP)){
  RDPindices=append(RDPindices,grep(sig.RDP[i],rnames)) #length=401
}
nonRDP=rnames[-RDPindices] #length=4246

false.neg.RDP.indices=c()
true.RDP=unique(c(RDP_Table[,1]))
for (i in 1:length(nonRDP)){
  false.neg.RDP.indices = append(false.neg.RDP.indices, c(grep(nonRDP[i],true.RDP, value=FALSE))) #2747 false negatives
}
true.neg.RDP.indices= length(nonRDP)-length(false.neg.RDP.indices) # 1499 true negatives

false.pos.rate.RDP=length(false.pos.RDP)/(true.neg.RDP.indices+length(false.pos.RDP)) # fals pos rate=6,08%
false.neg.rate.RDP=length(false.neg.RDP.indices)/(length(true.pos.RDP)+length(false.neg.RDP.indices))#false neg rate=90,0% 

#b) k-means 
clus4.RDP=c() #creating vector containing all RDPs identified by k-means in cluster 4
clus4.RDP.indices=c() #creating vector containing row number of clus5 identified RDPs
for (i in 1:nrow(mtrx.kmeans)){
if(clus5$cluster[i]==5){
  clus4.RDP=append(clus4.RDP, rnames[i])#length =551
  clus4.RDP.indices=append(clus4.RDP.indices,i)
}}

#find similar proteins between cluster4 and RDP_Table. Those are true positives.
true.pos.clus4=c()
true.pos.pos.clus4=c()
false.pos.clus4=c()
for (i in 1:length(clus4.RDP)){
  for (j in 1:nrow(RDP_Table)){
    if(clus4.RDP[i]==RDP_Table[j,1]){
      true.pos.clus4=append(true.pos.clus4,clus4.RDP[i]) #length=424 --> = true positives
      true.pos.pos.clus4=append(true.pos.pos.clus4, i) #position of true.pos in clus5.RDP
    }}}

false.pos.clus4=clus4.RDP[-true.pos.pos.clus4] #length =127 --> = false positives

nonRDP.clus4= rnames[-c(clus4.RDP.indices)] #length = 4105

false.neg.clus4.indices=c()
true.RDP=RDP_Table[,1]
for (i in 1:length(nonRDP.clus4)){
  false.neg.clus4.indices = append(false.neg.clus4.indices, c(grep(nonRDP.clus4[i],true.RDP, value=FALSE)))
} # 2634 false negatives 

true.neg.clus4.indices=length(nonRDP.clus4)-length(false.neg.clus4.indices) # 1471 true negatives

false.pos.rate.clus4=length(false.pos.clus4)/(true.neg.clus4.indices+length(false.pos.clus4)) # fals pos rate=7,95%
false.neg.rate.clus4=length(false.neg.clus4.indices)/(length(true.pos.clus4)+length(false.neg.clus4.indices))# false neg rate = 85,2% 
